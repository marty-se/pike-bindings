...
fn describe_pike_error(pike_error: &PikeThing) -> String {
    let pike_master = PikeObject::<()>::get_master();
...
impl fmt::Display for PikeError {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            ...
            PikeError::PikeError(pike_error) => {
                let desc = describe_pike_error(&pike_error);
                write!(f, "{}", desc)
            }
        }
    }
...
pub unsafe fn pike_error() -> ! {
    let pt = PikeThing::get_from_stack(-1);
    match pt {
...
where F: FnOnce() -> TRes {
    ...
    if setjmp_res != 0 {
        ...
        let thrown: PikeThing;
        unsafe {
            thrown = (&throw_value).into();
            throw_value.sub_ref();
            throw_value.mark_free();
        }
        call_res = Err(PikeError::PikeError(thrown))
    } else {
        call_res = Ok(closure());
    }
