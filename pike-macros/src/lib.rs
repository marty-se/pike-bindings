    ...
    for export in exports {
        ...
        output.push(
            quote! {
                ...
                pub unsafe extern "C" fn #export_ident(args: i32) -> #export_result {
        ...
        EXPORT_FUNC_INITS.with(|e| {
            ...
            a.push(
                quote! {
                    add_pike_func(#pike_ident, "function(mixed:mixed)", #export_ident);
                })
        });
    }
...
thread_local! {
    static EXPORT_FUNC_INITS: ::std::cell::RefCell<Vec<quote::Tokens>> = ::std::cell::RefCell::new(vec![]);
}
...
#[proc_macro]
pub fn init_pike_module(input: TokenStream) -> TokenStream {
    let mut inits: Vec<quote::Tokens> = vec![];
    EXPORT_FUNC_INITS.with(|e| { let ref v = &*e.borrow(); inits = v.clone().to_vec(); });
